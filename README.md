...
2) Add environment variable in your deployment platform:
...
## Admin: Loading overlay and toasts

- Full-screen loading overlay is shown during add, edit, and delete operations in `src/pages/Administration.js`.
- Styles live in `src/styles/Page Styles/Administration.module.css` under `.loadingOverlay` and `.loader` with keyframes `rotate` and `prixClipFix`.
- The overlay sits below the toast container (z-index 9998) so notifications are visible.

### Toast notifications (React-Toastify)
- Installed `react-toastify` and wired `ToastContainer` in `src/pages/_app.js` at top-center with high z-index.
- Success messages (Bulgarian):
  - Добавено успешно
  - Редактирано успешно
  - Изтрито успешно

## Heartbeat email alerts on failure

- Route: `src/pages/api/heartbeat.js`
  - Inserts into `public.heartbeat` and sends an alert email to `mdroshev@gmail.com` if Supabase errors or times out (>8s).
...
- To simulate failure and verify alert emails, temporarily change the table name in the route, call it, then revert the change.
